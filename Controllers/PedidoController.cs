using Microsoft.AspNetCore.Mvc;
using sastreria_data.Sources.BasedeDatos.Tablas;
using sastreria_data.Sources.BasedeDatos;

namespace SastreriaPresentación.Controllers
{
    [ApiController]
    [Route("api/pedido")]
    public class PedidoController : ControllerBase
    {
        private readonly SastreriaDbContext _db;
        private readonly ILogger<PedidoController> _logger;

        public PedidoController(SastreriaDbContext context, ILogger<PedidoController> logger)
        {
            _db = context;
            _logger = logger;
        }

        [HttpGet]
        [Route("")]
        public ActionResult<List<PedidoTable>> ListPedidos()
        {
            try
            {
                var pedidos = _db.Pedido.ToList();
                return Ok(pedidos);
            }
            catch (Exception ex)
            {
                _logger.LogError($"[PedidoController] error: {ex.Message} {ex.InnerException}");
                return StatusCode(500, "Error");
            }
        }

        //get x {id}
        [HttpGet("{id}")]
        public ActionResult<PedidoTable> GetPedidos(int id)
        {
            try
            {
                var pedido = _db.Pedido.Find(id);

                if (pedido == null)
                {
                    return NotFound();
                }

                return Ok(pedido);
            }
            catch (Exception ex)
            {
                _logger.LogError($"[PedidoController] error: {ex.Message} {ex.InnerException}");
                return StatusCode(500, "Error");
            }
        }
    }
}
